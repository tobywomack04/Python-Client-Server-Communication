Client and server communication (Python)
...
How to operate:<br>
Run the server script (echo-server1.py) on one machine/terminal and run the client script (echo-client1.py) on another terminal/machine. Enter a series of numbers (max 4) when prompted by the client. The client will then send these numbers to the server. The server checks whether the numbers are powers of two and sends back the results. The client will then display the results it received from the server.
